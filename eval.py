#  python evaluation/eval.py --model_name "unsloth/QwQ-32B"  --eval_file evaluation/data/1.json --port 8000 --max_new_tokens 4096 --batch_size 4 --task api
...
def main():
    ...
    client = openai.Client(base_url=r"http://gn1006:8000/v1", api_key="sk1234")
    #client = openai.Client(base_url=r"https://medusa-poc.genai.nchc.org.tw/v1", api_key="sk-")
    ...
    if args.strict_prompt:
        ...
        #query_prompt = "Select the most appropriate option(s). The correct answer may include one or more choices. Respond only with the corresponding letter(s) (e.g., A, B, C, etc.). If multiple answers are correct, list them without any additional text, explanation, or punctuation (e.g., A C D). Ensure that your response ends with the correct option in the format: 'The answer is A.' \n{question}\n{option_str}"  
        query_prompt = "Select the most appropriate option(s). The correct answer may include one or more choices. Respond only with the corresponding letter(s) (e.g., A, B, C). If multiple answers are correct, list them without punctuation or extra spaces (e.g., ACD). Think carefully but keep it simple. Your final response must follow this format: 'The answer is X.' \n**Question:** {question}\n**Options:** {option_str}"  
    else:
